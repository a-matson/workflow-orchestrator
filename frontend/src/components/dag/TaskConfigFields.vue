	...
	<div class="task-config-fields">
		...
		<template v-if="type === 'http_request'">
			<div class="field-row">
				<div class="field" style="flex: 0 0 90px">
					<label>Method</label>
					<select class="cf-input" :value="(cfg.method as string) ?? 'GET'"
				...
				<div class="field" style="flex: 1">
					<label>URL</label>
					<input class="cf-input" type="url" :value="cfg.url as string" placeholder="https://api.example.com/endpoint"
			...
			<div class="field">
				...
				<textarea class="cf-input cf-textarea cf-code" :value="headersStr"
					placeholder="Authorization: Bearer token&#10;Content-Type: application/json" rows="3"
			...
			<div class="field">
				...
				<div class="code-editor-wrap">
					...
					<textarea class="cf-input cf-textarea cf-code code-editor-ta" :value="bodyStr" placeholder='{"key": "value"}'
						rows="4" spellcheck="false" @input="patchJsonBody(($event.target as HTMLTextAreaElement).value)"
			...
			<div class="field-row">
				<div class="field" style="flex: 1">
					...
					<input class="cf-input" type="number" min="0" :value="(cfg.timeout_ms as number) ?? 10000"
		...
		<template v-else-if="type === 'database_query'">
			<div class="field">
				...
				<input class="cf-input cf-code" type="text" :value="cfg.connection_string as string"
			...
			<div class="field">
				...
				<div class="code-editor-wrap">
					...
					<textarea class="cf-input cf-textarea cf-code code-editor-ta" :value="cfg.query as string"
						placeholder="SELECT *&#10;FROM events&#10;WHERE created_at >= NOW() - INTERVAL '1 day'" rows="6"
						spellcheck="false" @input="patch('query', ($event.target as HTMLTextAreaElement).value)" />
				</div>
			</div>
			<div class="field-row">
				<div class="field" style="flex: 1">
					<label>Max Rows</label>
					<input class="cf-input" type="number" min="1" :value="(cfg.max_rows as number) ?? 10000"
		...
		<template v-else-if="type === 'data_transform'">
			<div class="field-row">
				<div class="field" style="flex: 1">
					...
					<select class="cf-input" :value="(cfg.input_format as string) ?? 'json'"
				...
				<div class="field" style="flex: 1">
					...
					<select class="cf-input" :value="(cfg.output_format as string) ?? 'json'"
			...
			<div class="field">
				...
				<div class="code-editor-wrap">
					...
					<textarea class="cf-input cf-textarea cf-code code-editor-ta" :value="cfg.script as string"
						placeholder="jq '. | map(select(.active))' /workspace/input.json > /workspace/output.json" rows="7"
						spellcheck="false" @input="patch('script', ($event.target as HTMLTextAreaElement).value)" />
				</div>
				<div class="editor-footer">
					<span class="footer-hint">💡 Files from artifact inputs are available at
		...
		<template v-else-if="type === 'ml_inference'">
			<div class="field">
				...
				<input class="cf-input cf-code" type="text" :value="cfg.model_name as string"
			...
			<div class="field-row">
				<div class="field" style="flex: 1">
					...
					<input class="cf-input cf-code" type="text" :value="cfg.input_path as string"
						placeholder="/workspace/input.json"
						@input="patch('input_path', ($event.target as HTMLInputElement).value)" />
				</div>
				<div class="field" style="flex: 1">
					<label>Output Path</label>
					<input class="cf-input cf-code" type="text" :value="cfg.output_path as string"
			...
			<div class="field-row">
				<div class="field" style="flex: 1">
					...
					<input class="cf-input" type="number" min="1" :value="(cfg.batch_size as number) ?? 32"
		...
		<template v-else-if="type === 'notification'">
			<div class="field">
				...
				<div class="notify-type-row">
					<button v-for="t in ['slack', 'webhook', 'email', 'pagerduty']" :key="t" class="notify-type-btn"
						:class="{ active: (cfg.notify_type ?? 'slack') === t }" @click="patch('notify_type', t)">
			...
			<div class="field">
				...
				<input class="cf-input" type="text" :value="cfg.channel as string" :placeholder="notifyChannelPlaceholder"
					@input="patch('channel', ($event.target as HTMLInputElement).value)" />
			</div>
			<div class="field">
				<label>Message</label>
				<textarea class="cf-input cf-textarea" :value="cfg.message as string"
					placeholder="Workflow completed successfully" rows="3"
		...
		<template v-else-if="type === 'gha_job'">
			<div class="field">
				...
				<input class="cf-input cf-code" :value="(cfg.runner_image as string) ?? 'fluxor/gha-runner:latest'"
			...
			<div class="field-row">
				<div class="field" style="flex: 1">
					<label>Runs-on</label>
					<input class="cf-input" :value="(cfg.runs_on as string) ?? 'ubuntu-latest'" readonly disabled />
				</div>
				<div class="field" style="flex: 1">
					<label>Job Key</label>
			...
			<div class="gha-steps-list">
				<div v-for="(step, i) in steps" :key="i" class="gha-step-card">
					<div class="step-card-header">
						...
						<input class="cf-input step-name-input" :value="step.name" placeholder="Step name"
					...
					<div class="step-type-row">
						<button class="step-type-btn" :class="{ active: !step.uses }" @click="setStepType(i, 'run')">
							run:
						</button>
						<button class="step-type-btn" :class="{ active: !!step.uses }" @click="setStepType(i, 'uses')">
							uses:
						</button>
					</div>
					...
					<template v-if="!step.uses">
						<div class="field" style="margin-top: 6px">
							...
							<div class="code-editor-wrap">
								...
								<textarea class="cf-input cf-textarea cf-code code-editor-ta" :value="step.run ?? ''"
									placeholder="npm ci&#10;npm test" rows="4" spellcheck="false"
						...
						<div class="field">
							...
							<input class="cf-input cf-code" :value="step.shell ?? 'bash'"
					...
					<template v-else>
						<div class="field" style="margin-top: 6px">
							...
							<input class="cf-input cf-code" :value="step.uses"
					...
					<details class="step-extras">
						...
						<div class="field" style="margin-top: 6px">
							...
							<textarea class="cf-input cf-textarea cf-code" :value="stepEnvStr(step.env)" rows="2" spellcheck="false"
								@input="patchStepEnv(i, ($event.target as HTMLTextAreaElement).value)" />
						</div>
						<div class="field-row">
							<div class="field" style="flex: 1">
								<label>if:</label>
								<input class="cf-input cf-code" :value="step.if ?? ''" placeholder="success()"
									@input="patchStep(i, 'if', ($event.target as HTMLInputElement).value)" />
							</div>
							<div class="field" style="flex: 1">
								<label>working-directory</label>
								<input class="cf-input cf-code" :value="step['working-directory'] ?? ''" placeholder="/workspace"
									@input="
			...
			<div class="editor-footer">
				<span class="footer-hint">💡 All steps share one container, <code>/workspace</code>, and env vars</span>
			</div>
		</template>
		...
		<template v-else>
			<div class="field">
				...
				<input class="cf-input cf-code" type="text" :value="cfg.command as string"
			...
			<div class="field">
				...
				<div class="code-editor-wrap">
					...
					<textarea class="cf-input cf-textarea cf-code code-editor-ta" :value="argsStr"
						placeholder="-c&#10;echo 'hello from fluxor'" rows="4" spellcheck="false"
			...
			<div class="field">
				...
				<div class="code-editor-wrap">
					...
					<textarea class="cf-input cf-textarea cf-code code-editor-ta" :value="envStr"
						placeholder="MY_VAR=hello&#10;DEBUG=true" rows="3" spellcheck="false"
						@input="patchEnv(($event.target as HTMLTextAreaElement).value)" />
				</div>
			</div>
			<div class="editor-footer">
				<span class="footer-hint">💡 Workspace artifacts available at <code>/workspace/&lt;path&gt;</code></span>
			</div>
		</template>
	</div>
</template>
