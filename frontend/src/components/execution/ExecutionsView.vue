      ...
      <template v-else>
        ...
        <div class="detail-progress-track">
          <div
            ...
            :style="{ width: completePct(selectedExec) + '%', background: statusColor(selectedExec.status) }"
          ></div>
        </div>
        ...
        <div class="detail-dag" ref="dagContainer">
          ...
          >
            <defs>
              <marker id="dv-arrow" markerWidth="7" markerHeight="5" refX="6" refY="2.5" orient="auto">
            ...
            >
              ...
              <rect
                ...
                rx="9" fill="#1a1a2e"
                :stroke="node.color" stroke-width="1"
              />
              <!-- Status bar -->
              <rect :x="node.x+1" :y="node.y+1" :width="NODE_W-2" height="3" :fill="node.color" rx="8" />
              <!-- Icon -->
              <text :x="node.x+10" :y="node.y+27" font-size="13" dominant-baseline="middle">{{ node.icon }}</text>
              <!-- Name -->
              <text
                :x="node.x+30" :y="node.y+20"
                fill="#e8e8f5" font-size="10.5" font-weight="600"
                font-family="DM Sans, sans-serif"
              >{{ node.label }}</text>
              <!-- Status text -->
              <text
                :x="node.x+30" :y="node.y+35"
                :fill="node.color" font-size="8.5"
                font-family="IBM Plex Mono, monospace"
              >{{ node.status.toUpperCase() }}</text>
              <!-- Retry badge -->
              <g v-if="node.retryCount > 0">
                <rect :x="node.x+NODE_W-30" :y="node.y+8" width="24" height="14" rx="4" fill="rgba(245,166,35,0.18)" />
                <text :x="node.x+NODE_W-18" :y="node.y+17" fill="#f5a623" font-size="9" font-family="IBM Plex Mono,monospace" text-anchor="middle">↺{{ node.retryCount }}</text>
              </g>
            </g>
          </svg>

          <style>
            @keyframes dagPulse { 0%,100%{stroke-opacity:.4} 50%{stroke-opacity:.95} }
            .dag-node-g { cursor: pointer; }
            .dag-node-g.selected rect:nth-child(2) { stroke-width: 2 !important; }
          </style>
        </div>
        ...
        <div class="task-list" ref="taskListEl">
          <div
            ...
            @click="selectedTaskDefId = selectedTaskDefId === t.task_definition_id ? null : t.task_definition_id"
          >
          ...
          <Transition name="expand">
            <div v-if="expandedTask" class="task-detail">
              <div class="task-detail-grid">
                ...
                <span class="td-val">{{ expandedTask.started_at ? new Date(expandedTask.started_at).toLocaleTimeString() : '—' }}</span>
...
import type { WorkflowExecution, TaskExecution, WorkflowStatus } from '../../types'
...
const expandedTask = computed(() =>
  selectedExec.value?.tasks.find(t => t.task_definition_id === selectedTaskDefId.value) ?? null
)
...
const dagNodePositions = computed(() => {
  ...
  const positions: Array<{
    ...
    status: string; color: string; retryCount: number
  }> = []
...
const dagEdges = computed(() => {
  ...
  dagNodePositions.value.forEach((n, i) => {
...
function elapsed(exec: WorkflowExecution) {
  ...
  const ms = (exec.completed_at ? new Date(exec.completed_at) : new Date(now.value)).getTime()
...
function taskDuration(t: TaskExecution) {
  ...
  const ms = (t.completed_at ? new Date(t.completed_at) : new Date(now.value)).getTime()
...
function taskStats(exec: WorkflowExecution) {
  ...
  exec.tasks?.forEach(t => { counts[t.status] = (counts[t.status] ?? 0) + 1 })
  return Object.entries(counts).map(([label, count]) => ({
    label, count, color: statusColor(label),
  }))
}

// Reset selectedTaskDefId when exec changes
watch(() => props.selectedId, () => { selectedTaskDefId.value = null })
</script>

<style scoped>
.executions-view { display: flex; height: 100%; overflow: hidden; }
...
.list-empty-icon { font-size: 28px; opacity: 0.3; }
...
.exec-item.active { background: rgba(124,106,255,.08); border-left-color: var(--accent); }

.exec-item-row1 { display: flex; align-items: center; gap: 7px; }
.exec-wf-name { font-size: 12px; font-weight: 600; flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.exec-elapsed { font-size: 10px; color: var(--text3); font-family: var(--mono); flex-shrink: 0; }

.exec-item-row2 { display: flex; align-items: center; gap: 8px; }
.mini-progress { flex: 1; height: 3px; background: var(--surface2); border-radius: 2px; overflow: hidden; }
.mini-progress-fill { height: 100%; border-radius: 2px; transition: width .4s; }
.exec-task-frac { font-size: 10px; color: var(--text3); font-family: var(--mono); flex-shrink: 0; }

.exec-item-row3 { display: flex; align-items: center; justify-content: space-between; }
.exec-id-mono { font-size: 9px; color: var(--text3); font-family: var(--mono); }
.exec-date { font-size: 9px; color: var(--text3); }
...
.detail-wf-name { font-size: 14px; font-weight: 600; }
.detail-exec-id { font-size: 10px; color: var(--text3); font-family: var(--mono); margin-top: 2px; }
.detail-header-right { display: flex; align-items: center; gap: 10px; }
.detail-elapsed { font-size: 12px; color: var(--text2); font-family: var(--mono); }
...
.hdr-btn.retry { color: var(--amber); border-color: rgba(245,166,35,.3); background: rgba(245,166,35,.08); }
.hdr-btn.retry:hover { background: rgba(245,166,35,.15); }
...
.stat-cell { flex: 1; padding: 8px 14px; border-right: 1px solid var(--border); }
.stat-cell:last-child { border-right: none; }
.stat-val { font-size: 18px; font-weight: 700; font-family: var(--mono); line-height: 1; }
.stat-lbl { font-size: 9px; color: var(--text3); text-transform: uppercase; letter-spacing: .07em; margin-top: 2px; }

/* Progress */
.detail-progress-track { height: 3px; background: var(--surface2); flex-shrink: 0; }
.detail-progress-fill { height: 100%; transition: width .5s ease; }
...
.task-list::-webkit-scrollbar-thumb { background: var(--border2); }
...
.task-row-selected { background: rgba(124,106,255,.06); }
.task-name { font-weight: 500; flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.task-type-tag { font-size: 9px; color: var(--text3); font-family: var(--mono); background: var(--surface); padding: 1px 6px; border-radius: 4px; flex-shrink: 0; }
.task-worker { font-size: 9px; color: var(--text3); font-family: var(--mono); width: 64px; text-align: right; flex-shrink: 0; }
.task-duration { font-size: 10px; color: var(--text2); font-family: var(--mono); width: 44px; text-align: right; flex-shrink: 0; }
.task-retry { font-size: 10px; color: var(--amber); flex-shrink: 0; }
...
.td-key { color: var(--text3); font-weight: 500; }
.td-val { color: var(--text2); font-family: var(--mono); }
.task-error {
...
.task-mini-logs {
  background: #0a0a12; border-radius: var(--radius-sm);
  padding: 6px; display: flex; flex-direction: column; gap: 2px;
  max-height: 130px; overflow-y: auto;
}
.mini-log { display: flex; gap: 7px; font-size: 10px; font-family: var(--mono); line-height: 1.6; }
.ml-ts { color: #4a5568; flex-shrink: 0; }
.ml-level { flex-shrink: 0; font-weight: 700; }
.mini-log.info .ml-level  { color: #63b3ed; }
.mini-log.warn .ml-level  { color: #ecc94b; }
.mini-log.error .ml-level { color: #fc8181; }
.ml-msg { color: #9fafc0; }

/* Transitions */
.exec-item-enter-active { transition: all .2s ease; }
.exec-item-enter-from { opacity: 0; transform: translateY(-4px); }
