	...
	<div class="timeline">
		...
		<Transition name="fade">
			<div v-if="hoveredTask" class="tl-tooltip">
				...
				<div class="tt-row">
					<span>Retries</span
					><span>{{ hoveredTask.retry_count }}/{{ hoveredTask.max_retries }}</span>
				</div>
				<div v-if="hoveredTask.started_at" class="tt-row">
					<span>Started</span><span>{{ fmt(hoveredTask.started_at) }}</span>
				</div>
				<div v-if="hoveredTask.completed_at" class="tt-row">
					<span>Completed</span><span>{{ fmt(hoveredTask.completed_at) }}</span>
				</div>
				<div v-if="hoveredTask.error" class="tt-error">
					{{ hoveredTask.error }}
				</div>
			</div>
		</Transition>
