  ...
  <div class="execution-dashboard">
    ...
    <div class="exec-header">
      ...
      <div class="exec-actions">
        <button @click="$emit('retry')" v-if="execution.status === 'failed'" class="btn-retry">↺ Retry</button>
        <button @click="$emit('cancel')" v-if="execution.status === 'running'" class="btn-cancel">◼ Cancel</button>
    ...
    <div class="task-metrics">
      <div v-for="stat in taskStats" :key="stat.status" class="stat-chip" :style="{ borderColor: stat.color, color: stat.color, background: stat.bg }">
      ...
      <div v-if="selectedTask" class="task-detail">
        ...
        <div class="detail-body">
          ...
          <div v-if="selectedTask.logs?.length" class="logs-preview">
            ...
            <div class="log-lines">
              <div v-for="(log, i) in selectedTask.logs.slice(-8)" :key="i" class="log-line" :class="log.level">
...
const props = defineProps<{ execution: WorkflowExecution; workflowTasks?: Array<{ id: string; dependencies: string[] }> }>()
const emit = defineEmits<{ (e: 'retry'): void; (e: 'cancel'): void; (e: 'task-click', task: TaskExecution): void }>()
...
const elapsed = computed(() => {
  ...
  const end = props.execution.completed_at ? new Date(props.execution.completed_at).getTime() : now.value
  const ms = end - start
...
const dagNodes = computed(() => {
  ...
  return tasks.map((t, i) => {
    ...
    return {
      ...
      data: { taskDef: { id: t.task_definition_id, name: t.task_name, type: t.task_type, dependencies: [], config: {} }, status: t.status, taskExec: t },
    }
  })
})
...
.exec-info { display: flex; align-items: center; gap: 10px; }
.exec-id { font-family: monospace; font-size: 11px; color: #9ca3af; }
.exec-name { font-size: 14px; font-weight: 600; color: #111827; }

.exec-actions { display: flex; gap: 8px; }
.btn-retry { padding: 5px 12px; background: #fef3c7; color: #b45309; border: 1px solid #fde68a; border-radius: 6px; font-size: 12px; cursor: pointer; }
.btn-cancel { padding: 5px 12px; background: #fee2e2; color: #dc2626; border: 1px solid #fca5a5; border-radius: 6px; font-size: 12px; cursor: pointer; }
.btn-toggle { padding: 5px 12px; background: #f9fafb; color: #6b7280; border: 1px solid #e5e7eb; border-radius: 6px; font-size: 12px; cursor: pointer; }

.progress-track {
  height: 3px; background: #f3f4f6; margin: 0;
}
.progress-fill {
...
.progress-fill.running { background: #3b82f6; animation: shimmer 1.5s infinite; }
...
.stat-count { font-weight: 700; font-size: 13px; }
.stat-label { text-transform: capitalize; }
.stat-chip.timing { background: #f8f9fa; color: #374151; border-color: #e5e7eb; }
...
.detail-name { display: flex; align-items: center; gap: 8px; font-size: 13px; font-weight: 600; }
.close-btn { background: none; border: none; cursor: pointer; color: #9ca3af; font-size: 16px; }

.detail-body { padding: 12px 16px; display: flex; flex-direction: column; gap: 8px; }
.detail-row { display: flex; justify-content: space-between; align-items: center; }
.detail-key { font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; color: #9ca3af; }
.detail-val { font-size: 12px; color: #374151; font-family: monospace; }

.error-block { display: flex; flex-direction: column; gap: 4px; }
.error-text { margin: 0; font-size: 11px; background: #fef2f2; color: #dc2626; padding: 8px; border-radius: 6px; white-space: pre-wrap; word-break: break-all; }

.logs-preview { display: flex; flex-direction: column; gap: 4px; }
.log-lines { background: #111827; border-radius: 8px; padding: 8px; max-height: 160px; overflow-y: auto; }
.log-line { display: flex; gap: 6px; font-family: monospace; font-size: 10px; line-height: 1.6; }
