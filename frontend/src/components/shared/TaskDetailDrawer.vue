		...
		<div v-if="task" class="drawer-backdrop" @click.self="$emit('close')">
			<div class="drawer">
				...
				<div class="meta-grid">
					...
					<div v-if="task.queued_at" class="meta-item">
					...
					<div v-if="task.next_retry_at" class="meta-item">
				...
				<div v-if="activeTab === 'logs'" class="drawer-logs">
					...
					<div ref="logListEl" class="log-list">
						...
						<div v-for="(entry, i) in filteredLogs" :key="i" class="log-row" :class="entry.level">
...
<script setup lang="ts">
	...
	const typeLabel = computed(
		() =>
			TASK_TYPES.find((t) => t.value === props.task?.task_type)?.label ??
	...
	const duration = computed(() => {
		...
		const ms =
			(t.completed_at ? new Date(t.completed_at) : new Date()).getTime() -
	...
	const formattedOutput = computed(() => {
		try {
			return JSON.stringify(props.task?.output, null, 2)
		} catch {
			return String(props.task?.output)
		}
	})
